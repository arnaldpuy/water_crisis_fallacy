---
title: "The Water Crisis by the Global Commission on the Economics of Water: A Totalising Narrative Built on Shaky Numbers"
subtitle: "R code"
author: "Arnald Puy"
header-includes:
...
hist.y <- dt.projections.ua %>%
  ...
  geom_histogram(colour = "black", alpha = 0.4, position="identity") +
...
```{r merge_figures2, dependson="all_model", fig.height=2, fig.width=4.5}

###########

plot_grid(hist.w, hist.y, ncol = 2, labels = "auto")
```

```{r merge_figures3, dependson="all_model", fig.height=2, fig.width=4.5}
...
```{r merge_figures4, dependson="rest", fig.height=2, fig.width=4.5}

########

plot_grid(plot_land_runoff + 
            scale_x_continuous(labels = scales::comma) +
            theme(plot.margin = unit(c(0.2, 0.45, 0.2, 0.2), "cm")),
          plot_accessible_runoff + 
            scale_x_continuous(labels = scales::comma), 
          ncol = 2)
```

```{r merge_figures5, dependson="rest", fig.height=5, fig.width=4.2, warning=FALSE}

########

top <- plot_grid(plot.irrigation, fraction.surface, labels = "auto")

bottom <- plot_grid(plot.exceedance + theme(legend.position = "none"), 
                    plot.exceedance.2050 + theme(legend.position = "none"), 
                    ncol = 2, 
                    labels = c("c", "d"))

legend <- get_legend(plot.exceedance + theme(legend.position = "top"))

legend.bottom <- plot_grid(legend, bottom, ncol = 1, rel_heights = c(0.15, 0.85))

final.up <- plot_grid(top, legend.bottom, ncol = 1)

final.down <- plot_grid(plot.caloric, plot.ind, ncol = 2, labels = c("e", "f"))

plot_grid(final.up, final.down, ncol = 1, rel_heights = c(0.65, 0.35))
```
